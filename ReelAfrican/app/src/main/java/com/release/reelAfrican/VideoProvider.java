...
public class VideoProvider {
    ...
    public static List<MediaInfo> buildMedia(String url) throws JSONException {
        ...
        String myjsonstring = url.toString();
        JSONObject jsonObjMain = new JSONObject(myjsonstring);
       // JSONObject jsonObj = new VideoProvider().parseUrl(url);
        JSONArray categories = jsonObjMain.getJSONArray(TAG_CATEGORIES);
        if (null != categories) {
                ...
                if (null != videos) {
                    for (int j = 0; j < videos.length(); j++) {
                        ...
                        for (int k = 0; k < videoSpecs.length(); k++) {
                            ...
                            if (TAG_MP4.equals(videoSpec.getString(TAG_VIDEO_TYPE))) {
                                videoUrl = videoSpec
                        ...
                        String dur = video.getString(TAG_DURATION);

                        int duration = Util.HoursToSecond(dur);
