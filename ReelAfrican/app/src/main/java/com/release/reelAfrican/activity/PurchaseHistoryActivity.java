...
import com.release.reelAfrican.adapter.PurchaseHistoryAdapter;
import com.release.reelAfrican.model.PurchaseHistoryModel;
import com.release.reelAfrican.model.RecyclerItemClickListener;
import com.release.reelAfrican.physical.OderDetailsActivity;
...
public class PurchaseHistoryActivity extends AppCompatActivity {
    ...
    String Invoice,Id,PutrcahseDate,TranactionStatus,Ppvstatus,Amount,Oid,Contenttype;
                ...
                new RecyclerItemClickListener(PurchaseHistoryActivity.this, new RecyclerItemClickListener.OnItemClickListener() {
                    ...
                    public void onItemClick(View view, int position) {
                        ...
                        if(purchaseData.get(position).getContenttype().equals("physical")){
                            ...
                            Util.ORDERID=purchaseData.get(position).getOid();
                            Util.invoiceid=purchaseData.get(position).getOrderId();
                            final Intent physical = new Intent(PurchaseHistoryActivity.this, OderDetailsActivity.class);
                            physical.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                            startActivity(physical);


                        }else {

                            final Intent digital = new Intent(PurchaseHistoryActivity.this, TransactionDetailsActivity.class);

                            digital.putExtra("id",Id_Purchase_History.get(position));
                            digital.putExtra("user_id",user_id);
                            digital.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                ...
                if (status > 0) {
                    if (status == 200) {
                        ...
                        for(int i=0 ;i<jsonArray.length();i++)
                        {
                            ...
                            if ((jsonArray.getJSONObject(i).has("order_id")) && jsonArray.getJSONObject(i).getString("order_id").trim() != null && !jsonArray.getJSONObject(i).getString("order_id").trim().isEmpty() && !jsonArray.getJSONObject(i).getString("order_id").trim().equals("null") && !jsonArray.getJSONObject(i).getString("order_id").trim().matches("")) {
                                Oid = jsonArray.getJSONObject(i).getString("order_id");

                            }
                            if ((jsonArray.getJSONObject(i).has("Content_type")) && jsonArray.getJSONObject(i).getString("Content_type").trim() != null && !jsonArray.getJSONObject(i).getString("Content_type").trim().isEmpty() && !jsonArray.getJSONObject(i).getString("Content_type").trim().equals("null") && !jsonArray.getJSONObject(i).getString("Content_type").trim().matches("")) {
                            ...
                            purchaseHistoryModel = new PurchaseHistoryModel(Invoice,Id,PutrcahseDate,TranactionStatus,Amount,Ppvstatus,Oid,Contenttype);
                            purchaseData.add(purchaseHistoryModel);
